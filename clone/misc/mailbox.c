...
void get_mail_text(mapping , string );
...
void send_mail(string rcvr, mapping mail)
{
	...
	//JackyBoy增加的通知用户有新邮件的程序
	//由于使用LOGIN_OB不能记录new_mail属性，故使用USER_OB来记录
	//由于/clone的euid为Domain，所以在/clone/user/user.c的query_save_file里多了一句：
	//if(id=="Domain") id=query("id");
	ob=new(USER_OB);
	ob->set("id",rcvr);
	ob->restore();
	ob->add("new_mail", 1);
	...
	if(ppl)
	{
		tell_object(ppl, BLINK+HIY "有人给你写信了哟！\n" NOR );
