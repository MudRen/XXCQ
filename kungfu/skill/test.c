...
{
    int i;
    for(i = sizeof(dodge_action)-1; i >= 0; i--)
    if(level >= dodge_action[i]["lvl"])
    return dodge_action[i]["skill_name"];
}

mapping query_dodge_action(object me, object weapon)
{
    int i, level;
    level = (int)me->query_skill("test", 1);
    for(i = sizeof(dodge_action); i > 0; i--)
    if(level > dodge_action[i-1]["lvl"])
    return dodge_action[NewRandom(i, 20, level/5)];
}
