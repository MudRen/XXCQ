...
int valib_parry(string usage) { return usage=="finger" || usage=="strike" || usage=="cuff";}
...
{
    int i;
    for(i = sizeof(parry_action)-1; i >= 0; i--)
    if(level >= parry_action[i]["lvl"])
    return parry_action[i]["skill_name"];
}

mapping query_parry_action(object me, object weapon)
{
    int i, level;
    level = (int)me->query_skill("test2", 1);
    for(i = sizeof(parry_action); i > 0; i--)
    if(level > parry_action[i-1]["lvl"])
    return parry_action[NewRandom(i, 20, level/5)];
}
