...
int ask_enter()
{
	...
	else if ( environment(ob)->query_temp("kfroom_4") )
	{
		command("say 现在有弟子在里面，你等一会吧。");
		return 1;
	}

	else if ( me->query("combat_exp") < 190000 )
	{
		command("say 你的功力不足,还是先去益武房打好基础吧。");
		return 1;
	}
	else if ( me->query("combat_exp") > 600000 )
	{
		command("say 你的功夫已经相当不错了，去穷武房进一步练功吧。");
		return 1;
	}
	else
	{
		...
		me->set_temp("kfroom_4",1);
		environment(me)->set_temp("kfroom_4",1);
		me->move("/d/shaolin/kfroom_4");
...
{
	if (environment(ob)->query_temp("kfroom_4"))
	{
		command("?");
		command("say 怎么还不出来呢？");
		command("say 算了，不管他了。");
		environment(ob)->delete_temp("kfroom_4");
	}
}