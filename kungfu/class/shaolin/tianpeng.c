...
void create()
{
	set_name("天鹏", ({"tian peng","tian","peng",}));
	set("long",
		"他是一位须发花白的老僧，身穿一袭金边黑布袈裟。他身材瘦高，\n"
		"太阳穴高高鼓起，似乎身怀绝世武功。\n"
	);

	set("nickname", WHT"接引罗汉"NOR);
	set("gender", "男性");
...
int ask_enter()
{
	...
	else if ( environment(ob)->query_temp("kfroom_1") )
	{
	...
	{
		command("say 你的功夫有一定根底，去进武房进一步练功吧。");
		return 1;
	}
	else
	{
		...
		message_vision(HIW"\n突然$N的身后闪出两个木人，将$N架进了始武房。\n"NOR,me);
		me->set_temp("kfroom_1",1);
		environment(me)->set_temp("kfroom_1",1);
		me->move("/d/shaolin/kfroom_1");
...
void check(object me,object ob)
{
	if (environment(ob)->query_temp("kfroom_1"))
	{
		command("?");
		command("say 怎么还不出来呢？");
		command("say 算了，不管他了。");
		environment(ob)->delete_temp("kfroom_1");
	}
}