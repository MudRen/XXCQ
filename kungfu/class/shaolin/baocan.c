...
void init()
{
	object ob = this_player();
        ...
        if(ob->query("title")=="少林长老")
          return;
      add_action("do_yun","yun");
      add_action("do_yun","exert");
       command("chat "+ob->query("name")+"想接受少林五长老的三招。\n");
...
int do_test()
{
	...
	{
	 command("say "+RANK_D->query_respect(ob)+"可惜你不是少林弟子!");
	 command("sigh");
	 return 1;
	}
  ...
  {
  command("say "+RANK_D->query_respect(ob)+"你不是少林掌门的弟子，回去吧!\n");
  ...
  return 1;
	...
	if (ob->query("qi") < ob->query("max_qi")/4)
	  {
	     command("chat "+ob->query("name")+"没有接下五老的第一招!\n");
	     return 1;
	  }
	 command("chat "+ob->query("name")+"接下五老的第一招!\n");
...
int do_test2()
{
	...
	if (ob->query("qi") < ob->query("max_qi")/4)
	  {
	     command("chat "+ob->query("name")+"没有接下五老的第二招!\n");
	     return 1;
	  }
	 command("chat "+ob->query("name")+"接下五老的第二招!\n");
...
int do_test3()
{
	...
	if (ob->query("qi") < ob->query("max_qi")/4)
	  {
	     command("chat "+ob->query("name")+"没有接下五老的第三招!\n");
	     return 1;
	  }
	 command("chat "+ob->query("name")+"接下五老的第三招!\n");
             call_out("halt",5);
...
void halt()
{
	...
	command("say 你能接我们少林五老三招不死，有前途，我们就把怀抱天下神功传授给你。\n"
		           "并且可以和我们平起平坐了,你就是抱字辈了!\n");
        ob->set("title","少林长老");
