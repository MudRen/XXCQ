...
void create()
{
	set_name("天刑", ({"tian xing",	"tian",	"xing",}));
	set("long",
...
int ask_enter()
{
	...
	else if ( environment(ob)->query_temp("kfroom_3") )
	{
	...
	{
		command("say 你的功力不足,还是先去进武房打好基础吧。");
		return 1;
	}
	else if ( me->query("combat_exp") > 200000 )
	{
		command("say 你的功夫相当不错，去精武房进一步练功吧。");
		return 1;
	}
	else
	{
		...
		message_vision(HIW"\n突然$N的身后闪出两个木人，将$N架进了益武房。\n"NOR,me);
		me->set_temp("kfroom_3",1);
		environment(me)->set_temp("kfroom_3",1);
		me->move("/d/shaolin/kfroom_3");
...
void check(object me,object ob)
{
	if (environment(ob)->query_temp("kfroom_3"))
	{
		command("?");
		command("say 怎么还不出来呢？");
		command("say 算了，不管他了。");
		environment(ob)->delete_temp("kfroom_3");
	}
}