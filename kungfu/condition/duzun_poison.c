...
int update_condition(object me, int duration)
{
        ...
        if(duration > 50 ) {
                ...
                me->receive_wound("qi",200-me->query_con());
                ...
                me->receive_wound("qi",150-me->query_con());
                me->receive_wound("jingli",75-me->query_con());
                me->start_busy(3);
        } else if (duration >10 ) {
                ...
                me->receive_wound("jingli",50-me->query_con());
                me->start_busy(2);
        } else {
                tell_object(me, YEL "你感觉到眼前景物晃动，全身疼痛得禁不住而抽搐起来。\n"+RED+"你中的「上天下地、惟我独尊」发作了。\n" NOR );
                message("vision",  BLK + me->name() + "的身子剧烈抽搐。\n"NOR, environment(me), me);
                me->receive_wound("qi",25 + random(25));
                me->receive_wound("jingli",25);
                me->start_busy(1);
        }
