...
void create()
{
	set("short", "益武房");
	set("long", @LONG
益武房是少林功夫房之一，内有精妙机关，置身其内可以很快的提升
...
void init()
{
	...
	if (userp(me) && me->query_temp("kfroom_3"))
...
int combat_go(object me, int x)
{
	...
	{
		if (x==11)
			message_vision(WHT"\n一走进益武房,四周的十数个木窗中便响起了卡卡的声音,似乎有机关开动了。\n"NOR, me);
		...
		ob->set_skill("force", consider_lv(ob, 10));
		ob->set_skill("dodge", consider_lv(ob, 10)); 
		ob->set_skill("club", consider_lv(ob, 10));
		ob->set_skill("parry", consider_lv(ob, 10)); 
		ob->move("/d/shaolin/kfroom_3");
...
{
	if( me->query_temp("kfroom_3") && dir == "east")
	{
		me->apply_condition("kf_job",15);
	}
