...
int main(object me, string arg)
{
        ...
        if(me->query("combat_exp") < 20000)
                         return notify_fail("就凭你那点武功，还想比武？\n");
        ...
        if (obj->query_temp("biwu") && me->query_temp("biwu"))//如果是比武的话
        {
                        if(me->query("combat_exp") < 20000)
                         return notify_fail("就凭你那点武功，还想比武？\n");
                        if(me->query("combat_exp") / 2 > obj->query("combat_exp"))
                        return notify_fail("你们两个的武功相差太大，根据规则，不能进行比武。\n");
        
                        if(obj->query("combat_exp") / 2 > me->query("combat_exp"))
                        return notify_fail("你们两个的武功相差太大，根据规则，不能进行比武。\n");

                        message("channel:chat",HIC"【江湖传闻】"+obj->query("name")+"和"
                        +me->query("name")+"在比武广场开始比武论剑！\n" NOR, users());
        }       
