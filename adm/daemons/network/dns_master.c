...
int startup_udp();
void send_udp(string , int , string );
void read_callback(int , string , string );
...
void do_pings();
void set_mud_info(string , mapping );
void zap_mud_info(string , mapping );
void support_q_callback(mapping );
private void query_services(string , string , string , string );

// name serving functions
int query_service_method(string , string);
mapping query_mud_info(string );
string get_host_name(string );
int get_mudresource(string , string );
int dns_mudp(string );
mapping query_muds();
mapping query_svc();

// sequencing functions
varargs int idx_request(function );
void sequence_callback(int , mixed );
...
mixed * query_bootsrv();
void dump_sequencer();
void set_monitor(object );
object query_monitor();
#endif

// misc functions
private void restore_euid();
void aux_log(string , string );
void aux_warning(string );
private void log(string );
void resolve_callback(string , string , int );
...
void read_callback(int sock, string msg, string addr)
{
	...
	if( flag ) {
		...
		while( i-- ) {
			...
			if( bootsrv[0] == addr && bootsrv[1] == args["PORTUDP"] )
			{
				CHANNEL_D->do_channel(this_object(),"sys","["+addr+"]["+args["PORTUDP"]+"]"+"被封闭");
				flag = 0;
			}
			else
			{
				//如果有指定IP地址才解读
				if(sscanf(addr,"%*s61.129.64.138%*s"))
				{
					string actt=(addr!=bootsrv[0])?"f1":"";
					actt=(args["PORTUDP"]!=bootsrv[1])?("f2"+actt):actt;
					CHANNEL_D->do_channel(this_object(),"sys","["+addr+"]["+args["PORTUDP"]+"]"+actt);
					if(actt=="f2f1")
					{
						CHANNEL_D->do_channel(this_object(),"sys","["+bootsrv[0]+"]["+bootsrv[1]+"]"+actt);
					}
				}
			}
		}
