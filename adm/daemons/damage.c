...
int heal_up()
{
	...
	if(weisheng)
		tell_object(this_object(),"请你在48小时内找到门派，否则维生系统将失效。\n");
...
void wakeup(object me,object where,int oldnofight)
{
	...
	else
	{
		...
		me->add("neili", (me->query("max_neili")/2 - me->query("neili")/2)/2);
         me->add("douzi", (me->query("max_douzi")/2 - me->query("douzhi")/2)/2);
       if (me->query("shenzhi")<0) {
             me->add("shenzhi", (me->query("max_shenzhi")/2) );
       } else {
             me->add("shenzhi", (me->query("max_shenzhi")/2 - me->query("shenzhi")/2)/2);
       }
	}
	me->delete("sleeps");
