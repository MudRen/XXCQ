...
int valid_shadow( object t) { return 0; }
...
int valid_override( string file, string name )
{
	...
	{
		log_file("efun","在文件"+file+"里有对"+name+"的efun::非法调用！\n");
		return 0;
	}
...
int valid_socket( object t1, string t2, mixed *t3 )
{
	return 1;
}
...
//   via LPC->C
int valid_compile_to_c( string t)
{
    return 1;
}
...
//   between paths
int valid_link( string t1, string t2)
{
    return 0;
}
...
//   (see config file)
int valid_save_binary( string  t)
{
    return 1;
}
...
int valid_bind(object binder, object t, object new_owner)
{
